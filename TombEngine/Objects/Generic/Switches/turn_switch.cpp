...
namespace TEN::Entities::Switches
{
	...
	void TurnSwitchCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
	{
			...
			laraInfo->Control.IsMoving && laraInfo->Context.InteractedItem == itemNumber)
		{
			if (TestLaraPosition(TurnSwitchBoundsA, switchItem, laraItem))
			{
				if (MoveLaraPosition(TurnSwitchPosA, switchItem, laraItem))
				{
					...
					g_Camera.ForcedFixedCamera.x = switchItem->Pose.Position.x - BLOCK(1) * phd_sin(switchItem->Pose.Orientation.y);
					g_Camera.ForcedFixedCamera.z = switchItem->Pose.Position.z - BLOCK(1) * phd_cos(switchItem->Pose.Orientation.y);
			...
			else
			{
				...
				if (TestLaraPosition(TurnSwitchBoundsC, switchItem, laraItem))
				{
					if (MoveLaraPosition(TurnSwitchPos, switchItem, laraItem))
					{
						...
						g_Camera.ForcedFixedCamera.x = switchItem->Pose.Position.x + 1024 * phd_sin(switchItem->Pose.Orientation.y);
						g_Camera.ForcedFixedCamera.z = switchItem->Pose.Position.z + 1024 * phd_cos(switchItem->Pose.Orientation.y);
		...
		if (doSwitch)
		{
			...
			g_Camera.ForcedFixedCamera.y = switchItem->Pose.Position.y - 2048;
	...
	void TurnSwitchControl(short itemNumber)
	{
		...
		if (switchItem->ItemFlags[1] == 1)
		{
			...
			g_Camera.UseForcedFixedCamera = 0;
			g_Camera.DisableInterpolation = true;
