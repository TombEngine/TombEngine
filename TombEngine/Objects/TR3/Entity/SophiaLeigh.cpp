...
namespace TEN::Entities::Creatures::TR3
{
	...
	static void SophiaLeighTowerControlFrame(ItemInfo& item, CreatureInfo* creature, SophiaData* data, AI_INFO& ai)
	{
		...
		{
			// FIX (Bug 3): Clamp to 0 rather than decrementing forever into large negative values.
			if (creature->Flags > 0)
				creature->Flags--;
		}
		...
		case SOPHIALEIGH_STATE_STAND:
			...
			// FIX (Bug 4): isValidTarget is now reliably true here because creature->Enemy
			// was restored to LaraItem before this frame runs (when ReachedGoal is true).
	...
	static void SophiaLeighTowerControl(ItemInfo& item, CreatureInfo* creature, SophiaData* data)
	{
		...
		// FIX (Bug 1 & 2): Set creature->Enemy to the current AI_X1 waypoint BEFORE calling
		// CreatureAIInfo, so that ai.distance and ai.ahead are computed relative to the
		// waypoint Sophia is navigating toward — not stale data from last frame.
		...
		if (creature->ReachedGoal)
		{
			...
			// FIX (Bug 2): Restore creature->Enemy to Lara BEFORE calling CreatureAIInfo so
			// that ai.verticalDistance is computed relative to Lara — which is what the
			// up/down tower decision actually needs. Previously ai was computed before
			// FindAITargetObject ran, so verticalDistance described the wrong target.
			...
			{
				// HYSTERESIS FIX: Only change waypoint after Lara has been consistently
				// on the same side for SOPHIALEIGH_WAYPOINT_STABLE_FRAMES frames.
				// This prevents oscillation when Lara is at roughly the same height.
				...
				// Determine which direction we'd like to go based on Lara's position.
				...
				else if (ai.verticalDistance < -SOPHIALEIGH_Y_DISTANCE_RANGE)
				{
					...
					if (creature->LocationAI > 0)
						...
						isInDeadZone = true; // Treat "can't go lower" as dead zone (no pending change).
				}

				// If we're in the dead zone (Lara at similar height, or can't move further),
				// clear any pending decision immediately rather than waiting for timer.
				if (isInDeadZone)
				{
					data->pendingLocationAI = -1;
					data->waypointStableTimer = 0;
				}
				// Otherwise apply hysteresis logic.
				else if (desiredLocationAI == data->pendingLocationAI)
				{
					// Same desired direction — decrement stability timer.
					data->waypointStableTimer--;

					// Once timer expires, commit to the new waypoint.
					if (data->waypointStableTimer <= 0)
					{
						creature->LocationAI = desiredLocationAI;
						data->pendingLocationAI = -1; // Clear pending state.
						data->waypointStableTimer = 0;
					}
				}
				else
				{
					// Direction changed — reset the timer and track new pending direction.
					data->pendingLocationAI = desiredLocationAI;
					data->waypointStableTimer = SOPHIALEIGH_WAYPOINT_STABLE_FRAMES;
				}
			}

			// Now run the rest of the frame's logic with ai relative to Lara.
		...
		else
		{
			...
			// Reset hysteresis state while navigating — we only decide on new waypoints
			// when actually standing at the current waypoint, not while in transit.
			data->pendingLocationAI = -1;
			data->waypointStableTimer = 0;

			// Still navigating toward waypoint — compute ai relative to the AI_X1 so that
			// movement decisions (walk/run transitions, turning) track the correct target.
			AI_INFO ai;
			CreatureAIInfo(&item, &ai);
	...
	void SophiaLeighControl(short itemNumber)
	{
		...
		if (item.HitPoints <= 0)
		{
			...
			if (item.Animation.FrameNumber >= endFrameNumber)
			{
				...
				// Do explosion effect.
				// IMPORTANT: allowExplosion=true so ExplodeBoss calls CreatureDie, which calls DropPickups.
