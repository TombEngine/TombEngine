...
namespace TEN::Entities::Traps
{
	...
	static void SpawnSparks(const Vector3& center, int roomNumber, float halfSpanX, float halfSpanZ,
							float cosY, float sinY, bool isWallMode, float wallHeight = 0.0f)
	{
		...
		if (isWallMode)
		{
			...
			Vector3 velocity = Vector3(-sinY * direction, Random::GenerateFloat(-0.2f, 0.2f), -cosY * direction);
		...
		else
		{
			...
			Vector3 velocity = Vector3(sin(angle), vertAngle + Random::GenerateFloat(-PI / 16, PI / 16), cos(angle));
			velocity.Normalize(velocity);
			spark.velocity = velocity * Random::GenerateFloat(8, 32);
		}

		spark.sourceColor      = Vector4(0.4f, 0.6f, 1.0f, 1);
	...
	static void SpawnFloorLight(const Vector3& center, int itemIndex, int roomNumber, float halfSpanX, float halfSpanZ,
								 float cosY, float sinY, bool isWallMode, float wallHeight = 0.0f)
	{
	...
	static bool IsEntityInField(const ItemInfo& item, const Vector3& center, const ItemInfo& entity,
								float halfSpanX, float halfSpanZ, float cosY, float sinY, bool isWallMode)
	{
	...
	static bool CheckCollisions(const ItemInfo& item, const Vector3& center, float halfSpanX, float halfSpanZ,
								float cosY, float sinY, bool isWallMode)
	{
	...
	static void DrawDebugVisualization(const ItemInfo& item, const Vector3& center, float halfSpanX, float halfSpanZ,
									   float cosY, float sinY, bool isWallMode, bool isKilling)
	{
