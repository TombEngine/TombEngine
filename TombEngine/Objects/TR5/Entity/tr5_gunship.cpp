...
namespace TEN::Entities::Creatures::TR5
{
	...
	constexpr int INERTIA_FRAMES = 15;
	...
	void ControlGunShip(short itemNumber)
	{
		...
		// Zustand bestimmen (ItemFlags[6] = Zustand, ItemFlags[4] = vorheriger Zustand, ItemFlags[5] = Inertia-Timer)
		int currentState = GunShipState::FOLLOW;
		if (hLen < minDistance)
		...
		if (isMoving && hLen > 100.0f)
		{
			...
			case GunShipState::FOLLOW:
				{
					// Ohne Offset: Nase zu Lara → TranslatePoint fliegt hin zu Lara
			...
			case GunShipState::EVADE_NEAR:
				{
					EulerAngles evadeOrient = targetOrient;
					...
					{
						const int floorH = GetFloorHeight(floorCheck, item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z);
						if (floorH != NO_VALUE)
						{
