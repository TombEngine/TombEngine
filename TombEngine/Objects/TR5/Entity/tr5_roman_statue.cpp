				...
				if (item->Animation.FrameNumber > 10)
				{
					...
					if (!item->TriggerFlags)
					{
						...
						if (item->Animation.FrameNumber == 34 && item->Animation.ActiveState == 3)
						{
							...
							auto shockPose1 = Pose(pos1);
							TriggerShockwave(&shockPose1, 16, 160, 96, 0, color / 2, color, 48, EulerAngles::Identity, 1, true, false, true, (int)ShockwaveStyle::Normal);
							TriggerRomanStatueShockwaveAttackSparks(pos1.x, pos1.y, pos1.z, 128, 64, 0, 128);

							pos1.y -= 64;

							auto shockPose2 = Pose(pos1);
							TriggerShockwave(&shockPose2, 16, 160, 64, 0, color / 2, color, 48, EulerAngles::Identity, 1, true, false, true, (int)ShockwaveStyle::Normal);
