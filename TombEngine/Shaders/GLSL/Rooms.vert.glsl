...
void main()
{
    ...
    vs_out.WorldPosition = pos;
    vs_out.Tangent = in_Tangent.xyz;
    vs_out.Binormal = cross(in_Normal.xyz, in_Tangent.xyz);
    vs_out.FaceNormal = in_FaceNormal.xyz;

    vs_out.FogBulbs = DoFogBulbsForVertex(vs_out.WorldPosition);
    vs_out.DistanceFog = DoDistanceFogForVertex(vs_out.WorldPosition);
}
