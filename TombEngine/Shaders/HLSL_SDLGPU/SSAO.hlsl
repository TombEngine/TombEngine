...
#define REG_CB_POST_PROCESS b1
// VS unused textures -> all t0/s0
#define REG_TEX_SSAO_DEPTH t0
#define REG_SMP_SSAO_DEPTH s0
#define REG_TEX_SSAO_NORMALS t0
#define REG_SMP_SSAO_NORMALS s0
#define REG_TEX_SSAO_NOISE t0
#define REG_SMP_SSAO_NOISE s0
#define REG_TEX_SSAO_INPUT t0
#define REG_SMP_SSAO_INPUT s0
#else
// PS CBs: Camera(0), PostProcess(7) -> contiguous b0-b1
#define REG_CB_CAMERA b0
#define REG_CB_POST_PROCESS b1
// PS textures -> contiguous t0-t3
#define REG_TEX_SSAO_DEPTH t0
#define REG_SMP_SSAO_DEPTH s0
#define REG_TEX_SSAO_NORMALS t1
#define REG_SMP_SSAO_NORMALS s1
#define REG_TEX_SSAO_NOISE t2
#define REG_SMP_SSAO_NOISE s2
#define REG_TEX_SSAO_INPUT t3
#define REG_SMP_SSAO_INPUT s3
#endif
...
Texture2D DepthTexture : register(REG_TEX_SSAO_DEPTH);
SamplerState DepthSampler : register(REG_SMP_SSAO_DEPTH);

Texture2D NormalsTexture : register(REG_TEX_SSAO_NORMALS);
SamplerState NormalsSampler : register(REG_SMP_SSAO_NORMALS);

Texture2D NoiseTexture : register(REG_TEX_SSAO_NOISE);
SamplerState NoiseSampler : register(REG_SMP_SSAO_NOISE);

Texture2D SSAOTexture : register(REG_TEX_SSAO_INPUT);
SamplerState SSAOSampler : register(REG_SMP_SSAO_INPUT);
