...
namespace TEN::Renderer
{
	...
	void Renderer::DrawOverlays(RenderView& view)
	{
		...
		else if (Lara.Control.Look.OpticRange != 0 && Lara.Control.Look.IsUsingLasersight)
		{
			...
			vertices[0].Color = PackColor(Vector4(1.0f, 0.0f, 0.0f, 1.0f));
			...
			vertices[1].Color = PackColor(Vector4(1.0f, 0.0f, 0.0f, 1.0f));
			...
			vertices[2].Color = PackColor(Vector4(1.0f, 0.0f, 0.0f, 1.0f));
			...
			vertices[3].Color = PackColor(Vector4(1.0f, 0.0f, 0.0f, 1.0f));
	...
	void Renderer::DrawDisplaySprites(RenderView& renderView, bool negativePriority)
	{
		...
		for (const auto& spriteToDraw : renderView.DisplaySpritesToDraw)
		{
			...
			for (int i = 0; i < rVertices.size(); i++)
			{
				...
				rVertices[i].Color = PackColor(Vector4(spriteToDraw.Color.x, spriteToDraw.Color.y, spriteToDraw.Color.z, spriteToDraw.Color.w));
			}
	...
	void Renderer::DrawFullScreenSprite(RendererSprite* sprite, DirectX::SimpleMath::Vector3 color, bool fit)
	{
		...
		vertices[0].UV.x = uvStart.x;
		vertices[0].UV.y = uvStart.y;
		vertices[0].Color = PackColor(Vector4(color.x, color.y, color.z, 1.0f));
		...
		vertices[1].UV.x = uvEnd.x;
		vertices[1].UV.y = uvStart.y;
		vertices[1].Color = PackColor(Vector4(color.x, color.y, color.z, 1.0f));
		...
		vertices[2].UV.x = uvEnd.x;
		vertices[2].UV.y = uvEnd.y;
		vertices[2].Color = PackColor(Vector4(color.x, color.y, color.z, 1.0f));
		...
		vertices[3].UV.x = uvStart.x;
		vertices[3].UV.y = uvEnd.y;
		vertices[3].Color = PackColor(Vector4(color.x, color.y, color.z, 1.0f));
