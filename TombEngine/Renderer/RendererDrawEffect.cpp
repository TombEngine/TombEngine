			...
			for (int i = 0; i < MAX_SPARKS_FIRE; i++) 
			{
				...
				if (spark->on)
				{
					...
					if (g_GameFlow->GetSettings()->Graphics.FlameHeatHaze)
						AddSpriteBillboard(&_sprites[Objects[ID_DEFAULT_SPRITES].meshIndex], position, NEUTRAL_COLOR, orientation, scalar, size * FLAME_HEAT_HAZE_SCALE, BlendMode::Distortion, true, view);
				}
			}
		}
	}
