...
void UpdatePlayerRefCameraOrient(ItemInfo& item)
{
	...
	bool isSpotCameraSwitch = (TEN::SpotCam::UseSpotCam != TEN::SpotCam::SpotcamSwitched);
...
void HandleLookAt(CameraInfo& camera, short roll)
{
	...
	// TODO: Develop's UpdateCameraMatrices does not accept roll/fov as parameters; both
	// are read from CameraInfo. Stash the spotcam roll for it via a future renderer pass.
...
// Legacy lifecycle entry points retained while the rest of the engine still drives the camera
// through PrepareCamera/UpdateCamera/TestLockedCamera. Implemented on top of g_Camera and develop's
// TEN::SpotCam API.
...
void PrepareCamera()
{
	if (TEN::SpotCam::TrackCameraInit)
	{
		TEN::SpotCam::UseSpotCam = false;
		SetFov(g_Camera.PrevFov);
