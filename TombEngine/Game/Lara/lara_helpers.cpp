...
bool HandleLaraVehicle(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (!g_Level.Items[lara->Context.Vehicle].Active)
	{
		...
		SetAnimation(*item, LA_FALL_START, 0, PLAYER_DEFAULT_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
		return false;
	}
...
{
	SetAnimation(*item, LA_FALL_BACK, 0, PLAYER_DEFAULT_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
	item->Animation.IsAirborne = true;
	item->Animation.Velocity.y = 0.0f;
}
...
void SetLaraMonkeyFallAnimation(ItemInfo* item)
{
	...
	SetAnimation(*item, LA_MONKEY_TO_FREEFALL, 0, PLAYER_DEFAULT_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
...
void SetLaraSlideAnimation(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (delta < -ANGLE(90.0f) || delta > ANGLE(90.0f))
	{
		...
		SetAnimation(*item, LA_SLIDE_BACK_START, 0, PLAYER_DEFAULT_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
	...
	else
	{
		...
		SetAnimation(*item, LA_SLIDE_FORWARD, 0, PLAYER_DEFAULT_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
...
void SetLaraSwimDiveAnimation(ItemInfo* item)
{
	...
	SetAnimation(*item, LA_ONWATER_DIVE, 0, PLAYER_DEFAULT_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseInOut);
