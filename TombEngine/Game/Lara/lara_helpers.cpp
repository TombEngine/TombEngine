...
bool HandleLaraVehicle(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (!g_Level.Items[lara->Context.Vehicle].Active)
	{
		...
		SetAnimation(*item, LA_FALL_START, 0, DEFAULT_PLAYER_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
		return false;
	}
...
void HandlePlayerFlyCheat(ItemInfo& item)
{
	...
	if (KeyMap[OIS::KeyCode::KC_O] && dbFlyCheat)
	{
		if (player.Context.Vehicle == NO_VALUE)
		{
			...
			{
				SetAnimation(item, LA_FLY_CHEAT, -0, DEFAULT_PLAYER_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
...
{
	SetAnimation(*item, LA_FALL_BACK, 0, DEFAULT_PLAYER_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
	item->Animation.IsAirborne = true;
	item->Animation.Velocity.y = 0.0f;
}
...
void SetLaraMonkeyFallAnimation(ItemInfo* item)
{
	...
	SetAnimation(*item, LA_MONKEY_TO_FREEFALL, 0, DEFAULT_PLAYER_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
...
void SetLaraSlideAnimation(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (delta < -ANGLE(90.0f) || delta > ANGLE(90.0f))
	{
		...
		SetAnimation(*item, LA_SLIDE_BACK_START, 0, DEFAULT_PLAYER_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
	...
	else
	{
		...
		SetAnimation(*item, LA_SLIDE_FORWARD, 0, DEFAULT_PLAYER_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseOut);
...
void SetLaraSwimDiveAnimation(ItemInfo* item)
{
	...
	SetAnimation(*item, LA_ONWATER_DIVE, 0, DEFAULT_PLAYER_ANIM_BLEND_FRAME_COUNT, BezierCurve2::EaseInOut);
