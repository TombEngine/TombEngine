...
namespace TEN::Renderer 
{
	...
	{
		using namespace TEN::Entities::Traps;
		using TEN::Renderer::ConstantBuffers::FogConeGPU;

		const auto& pool = GetTransientLightConePool();
		const float t = GetInterpolationFactor();

		// Volumetric-Fog-Cone-Liste für diesen Frame neu beginnen.
		BeginFogConesFrame();
		...
		for (int i = 0; i < MAX_TRANSIENT_LIGHT_CONES; ++i)
		{
			...
			// --------------------------------------------------------------------
			// (A) GPU-Daten für die volumetrische Füllung (Pixelshader, b15)
			// --------------------------------------------------------------------
			FogConeGPU gpu{};
			gpu.Apex = slot.Pos;

			auto axis = slot.Orientation.ToDirection();
			axis.Normalize(axis);

			gpu.Dir = axis;
			gpu.Len = (slot.MaxLength > 0.0f ? slot.MaxLength : 8192.0f);
			gpu.R0 = std::max(0.0f, slot.StartRadius);
			gpu.R1 = std::max(0.0f, slot.EndRadius);

			// Farbe/Parameter
			auto c = cone.Color; // Vector4
			gpu.Color = Vector3(c.x, c.y, c.z);
			gpu.Density = 0.035f;   // 0.02..0.06 je nach gewünschter Stärke
			gpu.G = 0.65f;    // Vorwärtsstreuung
			gpu.NoiseScale = 0.08f;    // sehr feines, stationäres Micro-Noise
			gpu.NoiseSpeed = 0.0f;     // kein "Wasserfall"-Gefühl

			PushFogConeGPU(gpu);

			// --------------------------------------------------------------------
			// (B) Hülle zeichnen (additiv), keine inneren Slices
			// --------------------------------------------------------------------
			// leichte, saubere Kontur – Alpha bewusst klein halten:
			Vector4 edgeColor = Vector4::Lerp(cone.OldColor, cone.Color, t);
			edgeColor.w *= 0.25f;
			...
			for (int s = 0; s < LightConeEffect::SUBDIVISION_COUNT; ++s)
			{
				...
				const auto v0 = Vector3::Lerp(cone.OldVertices[s], cone.Vertices[s], t);
				const auto v1 = Vector3::Lerp(cone.OldVertices[last ? 0 : (s + 1)],
					cone.Vertices[last ? 0 : (s + 1)], t);
				const auto v2 = Vector3::Lerp(cone.OldVertices[LightConeEffect::SUBDIVISION_COUNT + (last ? 0 : (s + 1))],
					cone.Vertices[LightConeEffect::SUBDIVISION_COUNT + (last ? 0 : (s + 1))], t);
				const auto v3 = Vector3::Lerp(cone.OldVertices[LightConeEffect::SUBDIVISION_COUNT + s],
					cone.Vertices[LightConeEffect::SUBDIVISION_COUNT + s], t);
		...
		// Einmal pro Frame die gesammelten Kegel hochladen/binden (PS b15).
		UploadFogConesCB();
	}
	...
	void Renderer::RenderVolumetricCones(RenderView& view)
	{/*
		using TEN::Effects::LightCone::GetTransientLightConePool;
		using TEN::Effects::LightCone::LightConeEffect;

		// Früher Exit: Targets vorhanden?
		if (!_volConeFrontZ.RenderTargetView || !_volConeBackZ.RenderTargetView)
			return;

		const auto& pool = GetTransientLightConePool();
		const float interp = GetInterpolationFactor();

		// Prüfen, ob überhaupt ein aktiver Kegel vorhanden ist.
		bool anyCone = false;
		for (int i = 0; i < MAX_TRANSIENT_LIGHT_CONES; ++i)
		{
			const auto& slot = pool[i];
			if (slot.On && slot.Life > 0.0f && slot.Effect.IsActive)
			{
				anyCone = true;
				break;
			}
		...
		// Gemeinsame Setup-States
		_shaders.Bind(Shader::Sprites);
		_context->IASetInputLayout(_inputLayout.Get());
		SetBlendMode(BlendMode::Opaque);    // wir schreiben Werte in ein Float-RT; kein Blending nötig
		SetDepthState(DepthState::None);    // keine Tiefenprüfung bei den Oberflächenprojektionen
		_context->RSSetViewports(1, &_viewport);

		// Lambda: einen Seiten-Quad (a0, a1, b1, b0) als 2 Dreiecke ausgeben.
		auto drawSideQuad = [&](const Vector3& a0, const Vector3& a1, const Vector3& b1, const Vector3& b0)
			{
				// einfache Normalen (für VS; in Depth-Pass egal, aber füllen wir sauber)
				Vector3 n = (a1 - a0).Cross(b1 - a0);
				n.Normalize();

				Vertex v0; v0.Position = a0; v0.Normal = n; v0.UV = Vector2(0, 0); v0.Color = Vector4(1, 1, 1, 1);
				Vertex v1; v1.Position = a1; v1.Normal = n; v1.UV = Vector2(1, 0); v1.Color = Vector4(1, 1, 1, 1);
				Vertex v2; v2.Position = b1; v2.Normal = n; v2.UV = Vector2(1, 1); v2.Color = Vector4(1, 1, 1, 1);
				Vertex v3; v3.Position = b0; v3.Normal = n; v3.UV = Vector2(0, 1); v3.Color = Vector4(1, 1, 1, 1);

				// 2 Dreiecke: (0,1,2) und (0,2,3)
				_primitiveBatch->DrawTriangle(v0, v1, v2);
				_primitiveBatch->DrawTriangle(v0, v2, v3);
			};

		// ---------------------------------------------------
		// PASS 1: FRONT-FACES -> _volConeFrontZ
		// ---------------------------------------------------
		{
			// Target setzen & clearen
			const float clear[4] = { 0,0,0,0 };
			_context->OMSetRenderTargets(1, _volConeFrontZ.RenderTargetView.GetAddressOf(), nullptr);
			_context->ClearRenderTargetView(_volConeFrontZ.RenderTargetView.Get(), clear);

			// Culling so setzen, dass **Frontfaces** gezeichnet werden.
			// (Bei standard CCW-Winding: Back cullen)
			SetCullMode(CullMode::CounterClockwise);

			// RenderType 4: "LightConeDepthFront"
			_stSprite.RenderType = 4;
			_cbSprite.UpdateData(_stSprite, _context.Get());

			_primitiveBatch->Begin();

			for (int idx = 0; idx < MAX_TRANSIENT_LIGHT_CONES; ++idx)
			{
				const auto& slot = pool[idx];
				const auto& cone = slot.Effect;
				if (!slot.On || slot.Life <= 0.0f || !cone.IsActive)
					continue;

				// Constant Buffer füllen (cbuffer CVolCone b8)
				_stVolCone.ConeApexWS = slot.Pos;

				auto dir = slot.Orientation.ToDirection();
				dir.Normalize(dir);
				_stVolCone.ConeDirWS = dir;

				_stVolCone.ConeLen = (slot.MaxLength > 0.0f ? slot.MaxLength : 8192.0f);
				_stVolCone.ConeR0 = slot.StartRadius;
				_stVolCone.ConeR1 = slot.EndRadius;

				_stVolCone.ConeColor = Vector3(cone.Color.x, cone.Color.y, cone.Color.z);
				_stVolCone.ConeG = 0.65f;

				_cbVolCone.UpdateData(_stVolCone, _context.Get());
				_context->PSSetConstantBuffers(8, 1, _cbVolCone.get());

				// Seitenflächen rendern (N Segmente)
				const int N = LightConeEffect::SUBDIVISION_COUNT;
				for (int s = 0; s < N; ++s)
				{
					const int j = (s + 1) % N;

					Vector3 a0 = Vector3::Lerp(cone.OldVertices[s], cone.Vertices[s], interp);
					Vector3 a1 = Vector3::Lerp(cone.OldVertices[j], cone.Vertices[j], interp);
					Vector3 b1 = Vector3::Lerp(cone.OldVertices[N + j], cone.Vertices[N + j], interp);
					Vector3 b0 = Vector3::Lerp(cone.OldVertices[N + s], cone.Vertices[N + s], interp);

					drawSideQuad(a0, a1, b1, b0);
				}
			}
		...
		// ---------------------------------------------------
		// PASS 2: BACK-FACES -> _volConeBackZ
		// ---------------------------------------------------
		{
			const float clear[4] = { 0,0,0,0 };
			_context->OMSetRenderTargets(1, _volConeBackZ.RenderTargetView.GetAddressOf(), nullptr);
			_context->ClearRenderTargetView(_volConeBackZ.RenderTargetView.Get(), clear);

			// Culling invertieren: **Front cullen**, damit nur Backfaces gezeichnet werden.
			SetCullMode(CullMode::Clockwise);

			// RenderType 5: "LightConeDepthBack"
			_stSprite.RenderType = 5;
			_cbSprite.UpdateData(_stSprite, _context.Get());
		...
		// ---------------------------------------------------
		// PASS 3: COMPOSITE (Fullscreen PS, liest FrontZ/BackZ/SceneDepth)
		// ---------------------------------------------------
		{
			_shaders.Bind(Shader::PostProcess);
			SetCullMode(CullMode::None);
			SetBlendMode(BlendMode::Additive); // oder Opaque – je nachdem, wie dein Composite-PS aufgebaut ist

			// Haupt-RT wieder setzen
			_context->OMSetRenderTargets(1, _renderTarget.RenderTargetView.GetAddressOf(), _renderTarget.DepthStencilView.Get());
			_context->RSSetViewports(1, &_viewport);

			// Front/Back Z + Szenentiefe binden (t7/t8/t6)
			BindRenderTargetAsTexture(TextureRegister::EnvironmentMapFront, &_volConeFrontZ, SamplerStateRegister::LinearClamp);
			BindRenderTargetAsTexture(TextureRegister::EnvironmentMapBack, &_volConeBackZ, SamplerStateRegister::LinearClamp);
			BindRenderTargetAsTexture(TextureRegister::DepthMap, &_depthRenderTarget, SamplerStateRegister::LinearClamp);

			// Cone-Parameter erneut an b8, falls der Composite-PS g, Farbe o.ä. pro Kegel nutzt (z. B. einheitlich)
			_context->PSSetConstantBuffers(8, 1, _cbVolCone.get());

			// Fullscreen zeichnen; dein PostProcess-PS muss t7/t8/t6 auswerten
			DrawFullScreenQuad(nullptr, Vector3::One, true);
		}

		// States optional zurücksetzen (Engine macht das meist später ohnehin in den regulären Pfaden)
