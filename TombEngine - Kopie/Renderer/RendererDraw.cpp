...
namespace TEN::Renderer
{
	...
	void Renderer::RenderScene(RenderTarget2D* renderTarget, RenderView& view, SceneRenderMode renderMode)
	{
		...
		PrepareSingleLightCone(view);
		PrepareFireflies(view);

		_suppressLightConeInMainPass = false; // zu Framebeginn
		...
		CalculateFrameRate();
		PrepareSingleLightCone(view);
		RenderVolumetricCones(view);
		_suppressLightConeInMainPass = true;  // danach nicht nochmal normal zeichnen
	}
