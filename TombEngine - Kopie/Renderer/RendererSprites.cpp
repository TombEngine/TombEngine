...
namespace TEN::Renderer
{
	...
	void Renderer::DrawSprites(RenderView& view, RendererPass rendererPass)
	{
		...
		for (const auto& spriteBucket : _spriteBuckets)
		{
			...
			if (_onlySpriteTypeEnabled && spriteBucket.RenderType != _onlySpriteType) continue;
			if (!_onlySpriteTypeEnabled && _suppressLightConeInMainPass && spriteBucket.RenderType == SpriteRenderType::LightCone) continue;
