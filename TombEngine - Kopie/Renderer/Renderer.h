...
#include "Renderer/ConstantBuffers/VolConeBuffer.h"
...
namespace TEN::Renderer
{
	...
	private:
		...
		// Volumetric cone front/back depth
		RenderTarget2D _volConeFrontZ; // R32_FLOAT
		RenderTarget2D _volConeBackZ;  // R32_FLOAT
		...
		//ConstantBuffer<CVolCone> _cbVolCone;
		// ---- Volumetric Fog Cones (an Kegel gebunden)
		TEN::Renderer::ConstantBuffers::CFogCones _stFogCones = {};
		ConstantBuffer<TEN::Renderer::ConstantBuffers::CFogCones> _cbFogCones;
		...
		void PrepareSingleLightCone(RenderView& view);
		...
		void RenderVolumetricCones(RenderView& view);
		void CreateVolConeTargets(int w, int h);
		...
		struct CVolCone {
			Vector3 ConeApexWS; float ConeLen;
			Vector3 ConeDirWS;  float ConeR0;
			Vector3 ConeColor;  float ConeR1;
			float ConeG;        Vector3 pad;
		};
		ConstantBuffer<CVolCone> _cbVolCone;
		CVolCone _stVolCone{};

		bool _onlySpriteTypeEnabled = false;
		SpriteRenderType _onlySpriteType = SpriteRenderType::Default;
		bool _suppressLightConeInMainPass = false;

		void BeginFogConesFrame() { _stFogCones.FogConeCount = 0; }
		void PushFogConeGPU(const TEN::Renderer::ConstantBuffers::FogConeGPU& c)
		{
			auto& S = _stFogCones;
			if (S.FogConeCount < 16)
				S.Cones[S.FogConeCount++] = c;
		}
		void UploadFogConesCB()
		{
			_cbFogCones.UpdateData(_stFogCones, _context.Get());
			_context->PSSetConstantBuffers(15, 1, _cbFogCones.get()); // b15
		}
