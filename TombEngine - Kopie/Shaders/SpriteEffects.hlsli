...
float4 DoLightConeEffect(float3 input, float4 output, float2 uv, float faceFactor, float timeUniform)
{
	// Ausgangsfarbe
    float4 color = output;

	// etwas animierter Noise für Staub im Licht
    float2 nUV = (uv - 0.5f) * 4.0f;
    float n1 = FractalNoise(nUV * 0.75f + timeUniform * 0.05f);
    float n2 = SimplexNoise(float3(nUV, timeUniform * 0.1f));
    float noise = saturate(0.6f * n1 + 0.4f * (n2 * 0.5f + 0.5f));

	// Radialer Falloff (bei Slices  Abstand zum Scheiben-Zentrum)
    float2 d = abs(uv - 0.5f) * 2.0f; // 0..1 in beide Richtungen
    float r2 = dot(d, d); // “Kreis” innerhalb Quad
    float radial = saturate(1.0f - r2); // viel Energie im Zentrum

	// Lngsverlauf (bei Seitenstreifen  entlang Achse)
    float axial = smoothstep(0.0f, 1.0f, uv.y) * (1.0f - smoothstep(0.6f, 1.0f, uv.y));

	// Kombinieren – für Slices zählt radial strker, für Seiten axial
    float vol = max(radial, axial);

	// leichte Schwankung
    vol *= lerp(0.85f, 1.15f, noise);

	// Fade an den Kanten (gegen harte Rnder der Quads)
    float edge = smoothstep(0.0f, 0.15f, uv.x) * smoothstep(0.0f, 0.15f, 1.0f - uv.x)
		* smoothstep(0.0f, 0.15f, uv.y) * smoothstep(0.0f, 0.15f, 1.0f - uv.y);

	// Gesichtsfaktor (wie bei den anderen Effekten, etwas milder)
    float fade = faceFactor * 0.5f;

    float boost = 1.0f + 0.25f * noise;
    float alpha = color.a * vol * edge * (0.85f + 0.15f * noise);
    float3 rgb = color.rgb * boost * (0.6f + 0.4f * vol);
